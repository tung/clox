...
SourceToChunk stmtContinue[] = {
  { "continue", false, LIST(uint8_t), LIST(Value) },
  { "continue;", false, LIST(uint8_t), LIST(Value) },
  { "while(nil)0;continue;", false, LIST(uint8_t), LIST(Value) },
  { "var x=true;while(x){x=nil;continue;print 0;}print 1;", true,
      LIST(uint8_t, OP_TRUE, OP_DEFINE_GLOBAL, 0, OP_GET_GLOBAL, 1,
          OP_JUMP_IF_FALSE, 0, 14, OP_POP, OP_NIL, OP_SET_GLOBAL, 2,
          OP_POP, OP_LOOP, 0, 13, OP_CONSTANT, 3, OP_PRINT, OP_LOOP, 0,
          19, OP_POP, OP_CONSTANT, 4, OP_PRINT, OP_RETURN),
      LIST(Value, S("x"), S("x"), S("x"), N(0.0), N(1.0)) },
  { "var x=true;for(;x;){x=nil;continue;print 0;}print 1;", true,
      LIST(uint8_t, OP_TRUE, OP_DEFINE_GLOBAL, 0, OP_GET_GLOBAL, 1,
          OP_JUMP_IF_FALSE, 0, 14, OP_POP, OP_NIL, OP_SET_GLOBAL, 2,
          OP_POP, OP_LOOP, 0, 13, OP_CONSTANT, 3, OP_PRINT, OP_LOOP, 0,
          19, OP_POP, OP_CONSTANT, 4, OP_PRINT, OP_RETURN),
      LIST(Value, S("x"), S("x"), S("x"), N(0.0), N(1.0)) },
  { "{var x=true;while(x){x=nil;continue;print 0;}print 1;}", true,
      LIST(uint8_t, OP_TRUE, OP_GET_LOCAL, 0, OP_JUMP_IF_FALSE, 0, 14,
          OP_POP, OP_NIL, OP_SET_LOCAL, 0, OP_POP, OP_LOOP, 0, 13,
          OP_CONSTANT, 0, OP_PRINT, OP_LOOP, 0, 19, OP_POP, OP_CONSTANT,
          1, OP_PRINT, OP_POP, OP_RETURN),
      LIST(Value, N(0.0), N(1.0)) },
  { "{var x=true;for(;x;){x=nil;continue;print 0;}print 1;}", true,
      LIST(uint8_t, OP_TRUE, OP_GET_LOCAL, 0, OP_JUMP_IF_FALSE, 0, 14,
          OP_POP, OP_NIL, OP_SET_LOCAL, 0, OP_POP, OP_LOOP, 0, 13,
          ...
          1, OP_PRINT, OP_POP, OP_RETURN),
      LIST(Value, N(0.0), N(1.0)) },
  { "var x;for(var i=0;i<6;i=i+1){x=!x;if(x)continue;print i;}", true,
      LIST(uint8_t, OP_NIL, OP_DEFINE_GLOBAL, 0, OP_CONSTANT, 1,
          OP_GET_LOCAL, 0, OP_CONSTANT, 2, OP_LESS, OP_JUMP_IF_FALSE, 0,
          40, OP_POP, OP_JUMP, 0, 11, OP_GET_LOCAL, 0, OP_CONSTANT, 3,
          OP_ADD, OP_SET_LOCAL, 0, OP_POP, OP_LOOP, 0, 23,
          OP_GET_GLOBAL, 5, OP_NOT, OP_SET_GLOBAL, 4, OP_POP,
          OP_GET_GLOBAL, 6, OP_JUMP_IF_FALSE, 0, 7, OP_POP, OP_LOOP, 0,
          26, OP_JUMP, 0, 1, OP_POP, OP_GET_LOCAL, 0, OP_PRINT, OP_LOOP,
          0, 36, OP_POP, OP_POP, OP_RETURN),
      LIST(Value, S("x"), N(0.0), N(6.0), N(1.0), S("x"), S("x"),
          S("x")) },
  { "for(var i=0;i<6;){var y;{var x=i;i=i+1;if(x<3)continue;print x;}}",
      true,
      LIST(uint8_t, OP_CONSTANT, 0, OP_GET_LOCAL, 0, OP_CONSTANT, 1,
          OP_LESS, OP_JUMP_IF_FALSE, 0, 38, OP_POP, OP_NIL,
          OP_GET_LOCAL, 0, OP_GET_LOCAL, 0, OP_CONSTANT, 2, OP_ADD,
          OP_SET_LOCAL, 0, OP_POP, OP_GET_LOCAL, 2, OP_CONSTANT, 3,
          OP_LESS, OP_JUMP_IF_FALSE, 0, 9, OP_POP, OP_POP, OP_POP,
          OP_LOOP, 0, 34, OP_JUMP, 0, 1, OP_POP, OP_GET_LOCAL, 2,
          OP_PRINT, OP_POP, OP_POP, OP_LOOP, 0, 46, OP_POP, OP_POP,
          OP_RETURN),
      LIST(Value, N(0.0), N(6.0), N(1.0), N(3.0)) },
};

COMPILE_STMTS(Continue, stmtContinue, 9);
